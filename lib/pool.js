...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function GetBlockTemplateParameters() {
        if (options.coin.passAlgorithm) {
            return [{ "capabilities": ["coinbasetxn", "workid", "coinbase/append"], "rules": ["segwit"] }, options.coin.passAlgorithm === true ? options.coin.algorithm : options.coin.passAlgorithm];
            ...
            return [{ "capabilities": ["coinbasetxn", "workid", "coinbase/append"], "rules": ["segwit"], "algo": options.coin.passAlgorithmKey === true ? options.coin.algorithm : options.coin.passAlgorithmKey }];
    ...
    function OnBlockchainSynced(syncedCallback) {
        ...
        var generateProgress = function () {
            ...
            _this.daemon.cmd(options.coin.getInfo ? 'getinfo' : 'getblockchaininfo', [], function (results) {
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        emitLog('Submitting block with ' + rpcCommand + ' to daemon');
        emitLog('Block Hex: ' + blockHex);
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function (results) {
                for (var i = 0; i < results.length; i++) {
                    ...
                    if (result.error) {
                        emitErrorLog('Error submitting block with ' + rpcCommand + ' ' +
                            JSON.stringify(result.error));
                        emitErrorLog('RPC Response: ' + JSON.stringify(result));
                        return;
                    }
                    else if (result.response === 'rejected') {
                        emitErrorLog('Daemon rejected block. Reason: ' + JSON.stringify(result.response));
                        // Add this line to get more details about the rejection
                        _this.daemon.cmd('getblocktemplate', [GetBlockTemplateParameters()[0]], function(templateResult) {
                            emitErrorLog('Current block template: ' + JSON.stringify(templateResult[0].response, null, 2));
                        });
                        return;
                    }
                    else {
                        emitLog('Daemon response to block submission: ' + JSON.stringify(result.response));
                    }
                }
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            if (_this.stratumServer) {
                var odoKey = blockTemplate.getOdoKey();
                if (odoKey) {
                    emitLog('[ODO DEBUG] Broadcasting new block with OdoKey: ' + odoKey.substring(0, 16) + '...');
                }
                _this.stratumServer.broadcastMiningJobs(blockTemplate.getJobParams(), odoKey);
            }
    ...
    function DetectCoinData(finishedCallback) {
        ...
        batchRpcCalls.push(
            ...
            [options.coin.noGetnetworkhashps ? 'getmininginfo' : options.coin.getAllNetworkHashPS ? 'getallnetworkhashps' : options.coin.getNetworkGHPS ? 'getnetworkghps' : options.coin.getNetworkGHPS ? 'getnetworkghps' : 'getnetworkhashps', []],
            ['submitblock', []]
        ...
        _this.daemon.batchCmd(batchRpcCalls, function (error, results) {
            ...
            options.protocolVersion = options.coin.getInfo ? rpcResults.getinfo.protocolversion : rpcResults.getnetworkinfo.protocolversion;

            var multiAlgoDifficultyKey = 'difficulty_' + (options.coin.passAlgorithm && options.coin.passAlgorithm !== true ? options.coin.passAlgorithm : options.coin.algorithm);
            options.initStats = {
                connections: rpcResults.getnetworkinfo.connections,
                difficulty: (options.coin.getInfo ? (typeof rpcResults.getinfo[multiAlgoDifficultyKey] !== 'undefined' ? rpcResults.getinfo[multiAlgoDifficultyKey] : (typeof (rpcResults.getinfo.difficulty) == 'object') ? rpcResults.getinfo.difficulty['proof-of-work'] : rpcResults.getinfo.difficulty) : (typeof rpcResults.getmininginfo[multiAlgoDifficultyKey] !== 'undefined' ? rpcResults.getmininginfo[multiAlgoDifficultyKey] : (typeof (rpcResults.getmininginfo.difficulty) == 'object') ? rpcResults.getmininginfo.difficulty['proof-of-work'] : rpcResults.getmininginfo.difficulty)) * algos[options.coin.algorithm].multiplier,
                networkHashRate: options.coin.noGetnetworkhashps ? rpcResults.getmininginfo.networkhashps : [options.coin.getAllNetworkHashPS ? rpcResults.getallnetworkhashps[options.coin.passAlgorithmKey ? (options.coin.passAlgorithmKey !== true ? options.coin.passAlgorithmKey : options.coin.algorithm) : (options.coin.passAlgorithm && options.coin.passAlgorithm !== true ? options.coin.passAlgorithm : options.coin.algorithm)] : options.coin.getNetworkGHPS ? (rpcResults.getnetworkghps * Math.pow(1024, 3)) : rpcResults.getnetworkhashps]
            };
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                var odoKey = _this.jobManager.currentJob.getOdoKey();
                var jobParams = _this.jobManager.currentJob.getJobParams();
                emitLog('[INITIAL JOB] Sending initial job after auth to ' + client.getLabel());
                emitLog('[INITIAL JOB] Job params: ' + JSON.stringify(jobParams));
                if (odoKey) {
                    emitLog('[INITIAL JOB] OdoKey present: ' + odoKey);
                } else {
                    emitLog('[INITIAL JOB] WARNING: No OdoKey for Odocrypt pool!');
                }
                this.sendMiningJob(jobParams, odoKey);
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function (result) {
                if (result.error) {
                    ...
                    // Check for odokey in the response
                    if (result.response && result.response.odokey) {
                        emitLog('[ODO DEBUG] GetBlockTemplate returned odokey: ' + result.response.odokey);
                    } else {
                        emitLog('[ODO DEBUG] WARNING: GetBlockTemplate did NOT return odokey!');
                    }
    ...
    function CheckBlockAccepted(blockHash, callback) {
        _this.daemon.cmd('getblock',
            ...
            function (results) {
                ...
                var logResults = results.map(function(result) {
                    return {
                        instance: result.instance ? result.instance.index : 'unknown',
                        error: result.error,
                        response: result.response ? {
                            hash: result.response.hash,
                            confirmations: result.response.confirmations,
                            height: result.response.height
                        } : null
                    };
                });
                emitLog('getblock response: ' + JSON.stringify(logResults, null, 2));
                ...
                if (validResults.length >= 1) {
                    emitLog('Block accepted: ' + blockHash);
                    callback(true, validResults[0].response.tx[0]);
                }
                else {
                    emitErrorLog('Block rejected: ' + blockHash);
                    // Log only relevant parts of the results for debugging
                    emitErrorLog('Rejection details: ' + JSON.stringify(logResults, null, 2));
                    callback(false);
                }
            }
        );
    ...
    **/
    this.processBlockNotify = function (blockHash, sourceTrigger) {
        ...
        if (!_this.jobManager) {
            emitErrorLog('Block notify called but jobManager not initialized yet');
            return;
        }
        if (_this.jobManager.currentJob && _this.jobManager.currentJob.rpcData && blockHash !== _this.jobManager.currentJob.rpcData.previousblockhash) {
            GetBlockTemplate(function (error, result) {
                if (error)
