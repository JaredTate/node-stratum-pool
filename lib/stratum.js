...
var StratumClient = function (options) {
    ...
    function handleMessage(message) {
        ...
        if (message.method !== 'mining.submit') {  // Don't log submits as they're too frequent
            emitWarningLog('[DEBUG] Received method: ' + message.method + ' params: ' + JSON.stringify(message.params));
        }
        ...
        switch (message.method) {
            ...
            case 'mining.configure':
                emitWarningLog('[ASICBOOST] mining.configure from ' + _this.remoteAddress);
                if (message.params) {
                    emitWarningLog('  Extensions: ' + JSON.stringify(message.params[0]));
                    if (message.params[1]) {
                        emitWarningLog('  Config: ' + JSON.stringify(message.params[1]));
                    }
                }
            ...
            case 'mining.submit':
                ...
                if (message.params.length > 5) {
                    emitWarningLog('[ASICBOOST] Submit with version bits: ' + message.params[5] + ' from ' + message.params[0]);
                }
    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask) && message.params.length > 5 && isValidHexRegex.test(message.params[5])) {
            var versionBits = parseInt(message.params[5], 16);
            var poolMask = parseInt(options.coin.version_mask, 16);
            
            // Add logging to see what's happening
            var versionBitsHex = versionBits.toString(16);
            while (versionBitsHex.length < 8) versionBitsHex = '0' + versionBitsHex;
            
            var versionBitsBinary = versionBits.toString(2);
            while (versionBitsBinary.length < 32) versionBitsBinary = '0' + versionBitsBinary;
            
            var poolMaskBinary = poolMask.toString(2);
            while (poolMaskBinary.length < 32) poolMaskBinary = '0' + poolMaskBinary;
            
            emitWarningLog('[ASICBOOST] Share from ' + (_this.workerName || 'unknown'));
            emitWarningLog('  Version bits received: 0x' + versionBitsHex);
            emitWarningLog('  Pool mask allows: ' + options.coin.version_mask);
            emitWarningLog('  Binary comparison:');
            emitWarningLog('    Version bits: ' + versionBitsBinary);
            emitWarningLog('    Pool mask:    ' + poolMaskBinary);
            
            // CORRECT validation: Check if miner used bits OUTSIDE the allowed mask
            var bitsOutsideMask = versionBits & (~poolMask);
            
            var bitsOutsideHex = bitsOutsideMask.toString(16);
            while (bitsOutsideHex.length < 8) bitsOutsideHex = '0' + bitsOutsideHex;
            
            emitWarningLog('  Bits outside mask: 0x' + bitsOutsideHex);
            emitWarningLog('  Validation: ' + (bitsOutsideMask === 0 ? 'PASS' : 'FAIL'));
            
            if (bitsOutsideMask !== 0) {
                sendJson({
                    id: message.id,
                    result: null,
            ...
            params.versionMask = versionBits;
        }
    ...
    function handleConfigure(message) {
        ...
        if (message.params && message.params[0] && message.params[0].includes('version-rolling')) {
            emitWarningLog('[ASICBOOST] Client requests version-rolling');
            
            if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask)) {
                response.result['version-rolling'] = true;
                response.result['version-rolling.mask'] = options.coin.version_mask;
                
                // Store client config if provided
                if (message.params[1]) {
                    _this.asicBoostConfig = message.params[1];
                    emitWarningLog('  Client config: ' + JSON.stringify(_this.asicBoostConfig));
                    
                    // Check min-bit-count if requested
                    if (_this.asicBoostConfig['version-rolling.min-bit-count']) {
                        var minBits = _this.asicBoostConfig['version-rolling.min-bit-count'];
                        var poolMask = parseInt(options.coin.version_mask, 16);
                        var availableBits = 0;
                        
                        // Count set bits in mask
                        for (var i = 0; i < 32; i++) {
                            if (poolMask & (1 << i)) availableBits++;
                        }
                        
                        emitWarningLog('  Client needs ' + minBits + ' bits, pool provides ' + availableBits + ' bits');
                        
                        if (availableBits < minBits) {
                            emitWarningLog('  WARNING: Cannot satisfy min-bit-count requirement');
                        }
                    }
                }
                
                emitWarningLog('  Response: version-rolling enabled with mask ' + options.coin.version_mask);
            } else {
                emitWarningLog('  ERROR: No version_mask in coin config!');
                response.result['version-rolling'] = false;
            }
        } else {
            emitWarningLog('[ASICBOOST] Client did not request version-rolling');
        }
    ...
    function setupSocket() {
        ...
        emitWarningLog('[DEBUG] New connection from ' + socket.remoteAddress);
        var dataBuffer = '';
