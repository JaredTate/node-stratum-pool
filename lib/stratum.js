...
var StratumClient = function (options) {
    ...
    function handleMessage(message) {
        switch (message.method) {
            ...
            case 'mining.configure':
                console.log('\n[ASICBOOST] mining.configure from', _this.remoteAddress);
                if (message.params) {
                    console.log('  Extensions:', JSON.stringify(message.params[0]));
                    if (message.params[1]) {
                        console.log('  Config:', JSON.stringify(message.params[1]));
                    }
                }
            ...
            case 'mining.submit':
                ...
                if (message.params.length > 5) {
                    console.log('[ASICBOOST] Submit with version bits:', message.params[5], 'from', message.params[0]);
                }
    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask) && message.params.length > 5 && isValidHexRegex.test(message.params[5])) {
            ...
            console.log('[ASICBOOST] Share from', _this.workerName || 'unknown');
            console.log('  Version bits received:', '0x' + versionBits.toString(16).padStart(8, '0'));
            console.log('  Pool mask allows:', options.coin.version_mask);
            console.log('  Binary comparison:');
            console.log('    Version bits:', versionBits.toString(2).padStart(32, '0'));
            console.log('    Pool mask:   ', poolMask.toString(2).padStart(32, '0'));
            
            // CORRECT validation: Check if miner used bits OUTSIDE the allowed mask
            var bitsOutsideMask = versionBits & (~poolMask);
            
            console.log('  Bits outside mask:', '0x' + bitsOutsideMask.toString(16).padStart(8, '0'));
    ...
    function handleConfigure(message) {
        ...
        if (message.params && message.params[0] && message.params[0].includes('version-rolling')) {
            console.log('[ASICBOOST] Client requests version-rolling');
            ...
            if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask)) {
                ...
                if (message.params[1]) {
                    ...
                    console.log('  Client config:', JSON.stringify(_this.asicBoostConfig));
                    ...
                    if (_this.asicBoostConfig['version-rolling.min-bit-count']) {
                        ...
                        console.log('  Client needs', minBits, 'bits, pool provides', availableBits, 'bits');
                        
                        if (availableBits < minBits) {
                            console.log('  WARNING: Cannot satisfy min-bit-count requirement');
                        }
                    }
                }
                
                console.log('  Response: version-rolling enabled with mask', options.coin.version_mask);
            } else {
                console.log('  ERROR: No version_mask in coin config!');
                response.result['version-rolling'] = false;
            }
        } else {
            console.log('[ASICBOOST] Client did not request version-rolling');
        }
