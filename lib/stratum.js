    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask) && message.params.length > 5 && isValidHexRegex.test(message.params[5])) {
            ...
            if (versionMask && ((~parseInt(options.coin.version_mask, 16)) & versionMask) !== 0) {
                _this.emit('log', 'debug', "Submitted version mask: " + message.params[5]);
                sendJson({
                    id: message.id,
                    result: null,
        ...
        _this.emit('submit', params, function (error, result) {
            if (error && params.versionMask !== undefined) {
                _this.emit('log', 'debug', "Share rejected with version mask: " + params.versionMask.toString(16));
            }
