    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && message.params.length > 5 && message.params[5]) {
            ...
            if (isValidHexRegex.test(cleanMask) && isValidHexRegex.test(cleanVersionBits)) {
                ...
                if (bitsOutsideMask !== 0) {
                    console.log('[ASICBOOST] Invalid submission from ' + (_this.workerName || 'unknown'));
                    console.log('  Version bits: 0x' + versionBits.toString(16));
                    console.log('  Bits outside allowed mask: 0x' + bitsOutsideMask.toString(16));
                }
        ...
        _this.emit('submit', params,
            function (error, result) {
                // Log rejections for ASICBoost shares
                if (!result && params.versionMask) {
                    console.log('[ASICBOOST] Share rejected for ' + _this.workerName);
