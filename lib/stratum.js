...
var StratumClient = function (options) {
    ...
    function handleMessage(message) {
        switch (message.method) {
            ...
            case 'mining.configure':
                console.log('[ASICBOOST] mining.configure from ' + _this.remoteAddress);
                if (message.params) {
                    console.log('  Requested extensions: ' + JSON.stringify(message.params[0]));
                    if (message.params[1]) {
                        console.log('  Parameters: ' + JSON.stringify(message.params[1]));
                    }
                }
    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && message.params.length > 5 && message.params[5]) {
            // Clean the mask and version bits of any 0x prefix
            var cleanMask = options.coin.version_mask.replace(/^0x/i, '');
            var cleanVersionBits = message.params[5].replace(/^0x/i, '');
            
            if (isValidHexRegex.test(cleanMask) && isValidHexRegex.test(cleanVersionBits)) {
                var versionBits = parseInt(cleanVersionBits, 16);
                var poolMask = parseInt(cleanMask, 16);
                
                // Log version mask details for debugging
                console.log('[ASICBOOST] Submit from ' + (_this.workerName || 'unknown'));
                console.log('  Version bits (param[5]): 0x' + versionBits.toString(16));
                console.log('  Pool allows mask: 0x' + poolMask.toString(16));
                
                // Check if miner used bits outside the allowed mask
                var bitsOutsideMask = versionBits & (~poolMask);
                console.log('  Bits outside mask: 0x' + bitsOutsideMask.toString(16));
                console.log('  Validation: ' + (bitsOutsideMask === 0 ? 'PASS' : 'FAIL'));
    ...
    function handleConfigure(message) {
        console.log('[ASICBOOST] handleConfigure called');
        
        // Check if client is requesting version-rolling
        var requestsVersionRolling = false;
        if (message.params && message.params[0]) {
            requestsVersionRolling = message.params[0].includes('version-rolling');
        }
        ...
        if (requestsVersionRolling && options.coin.version_mask) {
            // Remove 0x prefix if present for validation
            var cleanMask = options.coin.version_mask.replace(/^0x/i, '');
            
            if (isValidHexRegex.test(cleanMask)) {
                console.log('[ASICBOOST] Version-rolling supported, mask: ' + cleanMask);
                
                // Store client configuration for this session
                if (message.params && message.params[1]) {
                    _this.clientVersionConfig = message.params[1];
                    if (_this.clientVersionConfig['version-rolling.min-bit-count']) {
                        console.log('[ASICBOOST] Client requires min ' + _this.clientVersionConfig['version-rolling.min-bit-count'] + ' bits');
                    }
                }
                
                response.result['version-rolling'] = true;
                response.result['version-rolling.mask'] = cleanMask;  // Use clean mask without 0x
                response.result['version-rolling.min-bit-count'] = 16;  // BIP320 provides 16 bits
            }
        } else {
            console.log('[ASICBOOST] Version-rolling not supported or not requested');
            // Still send a valid response, just with empty result
            response.result['version-rolling'] = false;
        }
        
        console.log('[ASICBOOST] Sending response: ' + JSON.stringify(response));
        sendJson(response);
    }
