...
var StratumClient = function (options) {
    ...
    function handleMessage(message) {
        switch (message.method) {
            ...
            case 'mining.configure':
                ...
                if (message.params) {
                    emitLog('  Requested extensions: ' + JSON.stringify(message.params[0]));
                    if (message.params[1]) {
                        emitLog('  Parameters: ' + JSON.stringify(message.params[1]));
                    }
                }
    ...
    function handleConfigure(message) {
        ...
        if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask) && requestsVersionRolling) {
            ...
            if (message.params && message.params[1]) {
                ...
                if (_this.clientVersionConfig['version-rolling.min-bit-count']) {
                    emitLog('[ASICBOOST] Client requires min ' + _this.clientVersionConfig['version-rolling.min-bit-count'] + ' bits');
                }
            }
