...
var StratumClient = function (options) {
    ...
    function handleMessage(message) {
        switch (message.method) {
            ...
            case 'mining.configure':
                emitLog('[ASICBOOST] mining.configure from ' + _this.remoteAddress);
                if (message.params) {
                    emitLog('  Extensions: ' + JSON.stringify(message.params[0]));
                    if (message.params[1]) {
                        emitLog('  Config: ' + JSON.stringify(message.params[1]));
                    }
                }
            ...
            case 'mining.submit':
                ...
                if (message.params.length > 5) {
                    emitLog('[ASICBOOST] Submit with version bits: ' + message.params[5] + ' from ' + message.params[0]);
                }
    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask) && message.params.length > 5 && isValidHexRegex.test(message.params[5])) {
            ...
            emitLog('[ASICBOOST] Share from ' + (_this.workerName || 'unknown'));
            emitLog('  Version bits received: 0x' + versionBits.toString(16).padStart(8, '0'));
            emitLog('  Pool mask allows: ' + options.coin.version_mask);
            emitLog('  Binary comparison:');
            emitLog('    Version bits: ' + versionBits.toString(2).padStart(32, '0'));
            emitLog('    Pool mask:    ' + poolMask.toString(2).padStart(32, '0'));
            
            // CORRECT validation: Check if miner used bits OUTSIDE the allowed mask
            var bitsOutsideMask = versionBits & (~poolMask);
            
            emitLog('  Bits outside mask: 0x' + bitsOutsideMask.toString(16).padStart(8, '0'));
            emitLog('  Validation: ' + (bitsOutsideMask === 0 ? 'PASS ✓' : 'FAIL ✗'));
    ...
    function handleConfigure(message) {
        ...
        if (message.params && message.params[0] && message.params[0].includes('version-rolling')) {
            emitLog('[ASICBOOST] Client requests version-rolling');
            ...
            if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask)) {
                ...
                if (message.params[1]) {
                    ...
                    emitLog('  Client config: ' + JSON.stringify(_this.asicBoostConfig));
                    ...
                    if (_this.asicBoostConfig['version-rolling.min-bit-count']) {
                        ...
                        emitLog('  Client needs ' + minBits + ' bits, pool provides ' + availableBits + ' bits');
                        
                        if (availableBits < minBits) {
                            emitLog('  WARNING: Cannot satisfy min-bit-count requirement');
                        }
                    }
                }
                
                emitLog('  Response: version-rolling enabled with mask ' + options.coin.version_mask);
            } else {
                emitLog('  ERROR: No version_mask in coin config!');
                response.result['version-rolling'] = false;
            }
        } else {
            emitLog('[ASICBOOST] Client did not request version-rolling');
        }
