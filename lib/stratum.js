...
var StratumClient = function (options) {
    ...
    function handleSubmit(message) {
        ...
        if (options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask) && message.params.length > 5 && isValidHexRegex.test(message.params[5])) {
            ...
            emitLog('[ASICBOOST] Submit from ' + _this.workerName);
            emitLog('  Client version mask: 0x' + versionMask.toString(16));
            emitLog('  Pool allows mask: ' + options.coin.version_mask);
            emitLog('  Validation: ' + (((~poolMask) & versionMask) === 0 ? 'PASS' : 'FAIL'));
            
            if (versionMask && ((~poolMask) & versionMask) !== 0) {
                sendJson({
                    id: message.id,
                    result: null,
    ...
    function handleConfigure(message) {
        ...
        if (requestsVersionRolling && options.coin.version_mask && isValidHexRegex.test(options.coin.version_mask)) {
            console.log('[ASICBOOST] Version-rolling supported, mask: ' + options.coin.version_mask);
