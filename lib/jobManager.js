...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionMask) {
        var shareError = function (error) {
            emitErrorLog('Share error: ' + error[1]);
            _this.emit('share', {
                job: jobId,
                ip: ipAddress,
                worker: workerName,
        ...
        emitLog('Header hash: ' + headerHash.toString('hex'));

        var headerBigNum = bignum.fromBuffer(headerHash, { endian: 'little', size: 32 });
        emitLog('Header big number: ' + headerBigNum.toString());
        ...
        if (job.target.ge(headerBigNum)) {
            ...
            emitLog('Block found! Block hash: ' + blockHash);
            ...
            if (shareDiff / difficulty < 0.99) {
                ...
                if (previousDifficulty && shareDiff >= previousDifficulty) {
                    ...
                    emitLog('Low difficulty share. Share difficulty: ' + shareDiff + ', Miner difficulty: ' + difficulty);
        ...
        emitLog('Share data: ' + JSON.stringify({
            job: jobId,
            ip: ipAddress,
            port: port,
            worker: workerName,
