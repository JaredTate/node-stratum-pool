...
function debugLog(message) {
    fs.appendFileSync('/tmp/pool_debug.log', new Date().toISOString() + ': ' + message + '\n');
}
...
var JobManager = module.exports = function JobManager(options) {
    ...
    debugLog('Initializing JobManager');
    debugLog('Diff1: ' + diff1.toString(16));
    debugLog('ShareMultiplier: ' + shareMultiplier);
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName) {
        debugLog('Processing share - Job: ' + jobId + ', Difficulty: ' + difficulty + ', Worker: ' + workerName);
    
        var shareError = function (error) {
            debugLog('Share error: ' + JSON.stringify(error));
            _this.emit('share', {
                job: jobId,
                ip: ipAddress,
                worker: workerName,
        ...
        // var shareDiff = parseFloat((diff1 / Number(headerBigNum)) * shareMultiplier);

        var headerBigNum = bignum.fromBuffer(headerHash, {endian: 'little', size: 32});
        var shareDiff = diff1.div(headerBigNum).mul(shareMultiplier);
    
        debugLog('Share info: ' + JSON.stringify({
            shareDiff: shareDiff.toString(),
            difficulty: difficulty,
            ratio: shareDiff.div(bignum(difficulty)).toString()
        }));
    
        if (shareDiff.div(bignum(difficulty)).lt(0.99)) {
            debugLog('Low difficulty share: ' + shareDiff.toString());
