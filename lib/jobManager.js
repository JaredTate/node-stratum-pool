...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, versionMask) {
        ...
        function fromLittleEndian(buffer) {
            return BigInt('0x' + reverseBuffer(buffer).toString('hex'));
        }
        ...
        var merkleRoot = reverseBuffer(job.merkleTree.withFirst(coinbaseHash)).toString('hex');
        
        var headerBuffer = job.serializeHeader(merkleRoot, nTime, nonce, versionMask);
        var headerHash = hashDigest(headerBuffer, nTimeInt);
        var headerBigNum = fromLittleEndian(headerHash);
        
        logDebug(`headerBigNum: ${headerBigNum.toString()}`);
        ...
        logDebug(`diff1: ${diff1}`);
        logDebug(`shareMultiplier: ${shareMultiplier}`);
        logDebug(`Calculated shareDiff: ${shareDiff}`);
        logDebug(`Submitted difficulty: ${difficulty}`);

        var blockDiffAdjusted = job.difficulty * shareMultiplier;
        
        logDebug(`blockDiffAdjusted: ${blockDiffAdjusted}`);
        ...
        logDebug(`job.target type: ${typeof job.target}`);
        logDebug(`job.target value: ${job.target}`);
        logDebug(`headerBigNum type: ${typeof headerBigNum}`);
        logDebug(`headerBigNum value: ${headerBigNum}`);

        let comparison;
        try {
            comparison = BigInt(job.target) >= headerBigNum;
            logDebug(`Comparison result: ${comparison}`);
        } catch (error) {
            logDebug(`Error in BigInt comparison: ${error.message}`);
        }

        // Check if share is a block candidate (matched network difficulty)
        if (BigInt(job.target) >= headerBigNum) {
            logDebug(`Block candidate found! job.target: ${job.target}, headerBigNum: ${headerBigNum}`);
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, nTime).toString('hex');
        } else {
            logDebug(`Not a block candidate. job.target: ${job.target}, headerBigNum: ${headerBigNum}`);
            if (options.emitInvalidBlockHashes)
                blockHashInvalid = reverseBuffer(sha256d(headerBuffer)).toString('hex');
        
            // Check if share didn't reach the miner's difficulty
            if (shareDiff / difficulty < 0.99) {
                logDebug(`Low difficulty share. shareDiff: ${shareDiff}, difficulty: ${difficulty}, ratio: ${shareDiff / difficulty}`);

                // Check if share matched a previous difficulty from before a vardiff retarget
                if (previousDifficulty && shareDiff >= previousDifficulty) {
                    logDebug(`Share matched previous difficulty. previousDifficulty: ${previousDifficulty}`);
                    difficulty = previousDifficulty;
                } else {
                    logDebug(`Share rejected due to low difficulty`);
