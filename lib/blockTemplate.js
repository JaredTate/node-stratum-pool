...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients, network, emitLog) {
    ...
    var coinbaseBuffer = this.serializeCoinbase(new Buffer('00'.repeat(this.generationTransaction[0].length), 'hex'), new Buffer('00'.repeat(this.generationTransaction[1].length), 'hex'));
    var coinbaseHash = util.sha256d(coinbaseBuffer);
    
    if (rpcData.transactions.length === 0) {
        this.merkleRoot = util.reverseBuffer(coinbaseHash).toString('hex');
    } else {
        this.merkleTree = new merkleTree(getTransactionBuffers(rpcData.transactions));
        this.merkleBranch = getMerkleHashes(this.merkleTree.steps);
        this.merkleRoot = util.reverseBuffer(this.merkleTree.withFirst(coinbaseHash)).toString('hex');
    }
    emitLog('Calculated merkle root: ' + this.merkleRoot);

    this.serializeHeader = function(nTime, nonce){
        emitLog('Serializing header with:');
        emitLog(`merkleRoot: ${this.merkleRoot}`);
        emitLog(`nTime: ${nTime}`);
        emitLog(`nonce: ${nonce}`);
